...
@services_router.message(F.text == 'SERVICES Заказы ожидающие рассчета')
async def all_services_orders_wait_payment(message: types.Message, session: AsyncSession, bot: Bot):
    count = 0
    if message.from_user.id in bot.my_admins_list:
        try:
            orders = await orm_get_all_current_services_order(session)
            for order in orders:
                ...
                if user is None:
                    await message.answer(
                        ...
                        reply_markup=get_callback_btns(btns={
                            'Оплачено': f'SERVICESpayment_{order.id}',
...
@services_router.callback_query(F.data.startswith('SERVICESpayment_'), StateFilter(None))
...
async def add_amount_for_order_shop(message: types.Message, state: FSMContext, session: AsyncSession, bot: Bot):
    ...
    data = await state.get_data()
    try:
        await orm_update_order(session=session, amount=data['amount'], order_id=int(data['order_id']))
...
@services_router.callback_query(F.data.startswith('SERVICESmessage_'), StateFilter(None))
...
@services_router.callback_query(F.data.startswith('SERVICEScancel_'))
...
@services_router.callback_query(F.data.startswith('SERVICEcomplete_'))
